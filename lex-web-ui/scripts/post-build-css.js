...
const viteCssFile = path.join(bundleDir, 'lex-web-ui.css');
...
if (!fs.existsSync(sourceCssFile)) {
  if (isProd && fs.existsSync(viteCssFile)) {
    fs.renameSync(viteCssFile, sourceCssFile);
    console.log(`✓ Renamed ${path.basename(viteCssFile)} to ${path.basename(sourceCssFile)}`);
  } else {
