...
popd
aws s3 cp --profile org-account out/src-$version.zip \
  "s3://${BOOTSTRAP_BUCKET_PATH}/src-$version.zip"

aws s3 cp --profile org-account out/custom-resources-$version.zip \
  "s3://${BOOTSTRAP_BUCKET_PATH}/custom-resources-$version.zip"

aws s3 cp --profile org-account out/initiate-chat-lambda-$version.zip \
  "s3://${BOOTSTRAP_BUCKET_PATH}/initiate-chat-lambda-$version.zip"

aws s3 cp --profile org-account out/get-signed-url-lambda-$version.zip \
  "s3://${BOOTSTRAP_BUCKET_PATH}/get-signed-url-lambda-$version.zip"

aws s3 sync --profile org-account --exclude "*" --include "*.yaml" \
